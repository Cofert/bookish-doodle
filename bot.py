...
async def fetch(
    ...
    h = {"User-Agent": "Mozilla/5.0 (compatible; RoHelperBot/1.0)"}
    if headers:
        h.update(headers)
    kwargs = {"headers": h, "timeout": aiohttp.ClientTimeout(total=15)}
    if use_proxy:
...
async def scrape(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not item_data:
            await interaction.followup.send(f"❌ Couldn't find **{item}** on Rolimons. Check spelling and try again.")
            return
